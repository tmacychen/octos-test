...
class TestMatrixMessageDedup:
    ...
    def test_duplicate_event_id_ignored(self, runner):
        ...
        dedup_event_id = f"$dedup_{uuid.uuid4().hex[:12]}"
        dedup_room = f"!dedup_{uuid.uuid4().hex[:8]}:localhost"

        # 第一次发送，应收到回复
        reply1 = inject_and_get_reply(runner, "Dedup test", timeout=TIMEOUT_LLM,
                                       room_id=dedup_room, event_id=dedup_event_id)
        ...
        # 第二次发送相同 event_id
        runner.inject("Dedup test", room_id=dedup_room, event_id=dedup_event_id)
...
class TestMatrixAllowedSenders:
    """allowed_senders 白名单过滤测试"""
    ...
    def test_blocked_sender_no_reply(self, runner):
        """白名单外用户发送消息 → bot 不回复"""
        count_before = len(runner.get_sent_messages(timeout=5))
        runner.inject("Hello from stranger", sender="@stranger:notallowed.com")
